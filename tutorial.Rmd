...
Convert one of the apps above to a Shiny app directory, or write your own app. Name the directory using your own name, as we are going to upload it to a server.
...
Upload your app to our server with `scp`. Your instructor will give you details for how to do this.
...
A Shiny module consists of a function create the UI and function to be called with `callModule` in the server function.
...
* A Shiny module's server function can return a value, which could be a list containing reactive expressions.
