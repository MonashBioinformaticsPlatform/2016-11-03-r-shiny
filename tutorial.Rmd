...
Please make sure you are running R 3.3 and Bioconductor 3.3 or higher. Bioconductor 3.3 in particular is vastly more stable than 3.2. <!-- You can force Bioconductor to upgrade by running `remove.packages("BiocInstaller")`, restarting R, and then following the code above. -->
...
( Shiny [supports bookmarking](http://shiny.rstudio.com/articles/bookmarking-state.html), but it isn't enabled by default and requires UI code to be wrapped in `function(request) ...`. )
